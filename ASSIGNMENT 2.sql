...
Select distinct city from station where city not like 'A%A' OR city not like 'E%E' OR city not like 'I%I' OR city not like 'O%O' OR city not like 'U%U';

select * from emp;

select * from emp where year(hire_date) >=2020 and salary >2000 order by emp_no;
...
select avg(district) population  from city order by population ;
